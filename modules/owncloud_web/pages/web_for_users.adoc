...
ownCloud Web offers contextual help. In those cases where possible, you will see a questionmark as shown in the image below:

image::web-for-users/share-via-link.png[Sharing,width=400]

When clicking on the questionmark, a context specific frame with additional help text shows up which can be closed by clicking outside, see the image below for an example.
