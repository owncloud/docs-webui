...
Under `Personal` in the left sidebar you have your private space where you can upload files and folders or create them directly on oCIS.

image:personal-space.png

You can also share files either via public link or internally with others users on the same oCIS instance by invitation. Either way, you can grant permission to access files as Viewer or Editor.
...
Sharing is either done via the icons to the right of a file or folder or via the `Actions` menu or by opening the right sidebar and clicking `Shares`. In any case, the right sidebar provides you a dialog where you can chose between inviting people registered on the oCIS instance by entering their names or email addresses in the `Invite` field and click btn:[Share] or by sharing via link.
...
If you modified a document stored in oCIS, you can always restore older versions. When collaborating with other people on the same documents, versioning is very important. If you share a document with an editor, you want to be able to check what happened with the document.
