...
Under `Shares`, you can see all files or folders that have been shared with you. The origin of the share always resides in a space, see below. You can accept the share or decline. You can see who the owner of the share is and who else it has been shared with. Then there's the time or date when the content has been shared.
...
Spaces are the foundation for sharing data. Spaces are not owned by someone. Spaces store data and are designed for collaboration. Spaces first need to be created by a user having the `Create Space` permission. (Ask your admin for details). The xref:sharing-roles-and-permissions[Manager role] for managing the space can further be delegated to one or more users, where the creator can remove himself from this role completely if required.

Spaces are organized by the team members themselves without intervention from admins. Leaving members do not interupt sharing and other sharees can keep on working. This is true not only for the whole space or for single files or directories, but also for roles and permissions assigned.

Whole spaces either show up as `Personal` space or in the `Spaces` area, while when content only is shared, it is shown in `Shared` making it easier to distinguish the way of sharing respectively viewing and assiging sharing roles and permissions.

==== Space Areas

Personal::
Every user has a private space, where he can store or share content. This space is created automatically when the user has been setup. Delegating the managing role or adding members to the whole personal space is not possible though sharing single files or directories is enabled. With the inability to assign or delegate the managing role or add space members, the personal space stays in total control of the user.
+
NOTE: Consider using the personal space for personal and unshared data only and have additional space(s) for data that is sharable. This increases security a lot and prevents accidentially providing private data to the outside. In addition, when leaving the company, sharing stays untouched and private data can safely be deleted. 

Spaces::
Spaces where a user is member of show up here. This area is intended for team collaboration, like when sharing on a larger scale, working together on a project, in a department or in a school class.
...
* Members can be added to a Space so that they gain access to all files and folders within the space. Members get the sharing *and* space permissions granted depending by their role.
...
The following table describes the default roles and it's permissions.

NOTE: Permissions *sum up* as higher the role gets.
...
* recover files from the spaces trashbin.
...
(1) ... With deactivating, a Manager can disable any access to a space including sharing and syncing. This can be used to keep data in a frozen state before finally deleting it later on. To regain full access including sharing and syncing, a deactivated space can be activated again.
